...
func SlbSecDetail(ctx context.Context, client *sdk.Client, req *SlbSecDetailRequest) ([]SlbSecDetailItem, error) {
	...
	for i, item := range result.Items {
		...
		if item["tenor"] == nil {
			// 字段值为 null，使用零值
			tenor = ""
		} else if v, ok := item["tenor"].(string); ok {
			tenor = v
		} else if v, ok := item["tenor"].(float64); ok {
			tenor = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["tenor"].(int); ok {
			tenor = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["tenor"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: slb_sec_detail")
			log.Printf("字段: tenor")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["tenor"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
