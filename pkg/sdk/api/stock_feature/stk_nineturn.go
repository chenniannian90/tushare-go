...
func StkNineturn(ctx context.Context, client *sdk.Client, req *StkNineturnRequest) ([]StkNineturnItem, error) {
	...
	for i, item := range result.Items {
		...
		// 处理 float64 类型 - 支持多种输入格式
		var upCount float64
		if item["up_count"] == nil {
			// 字段值为 null，使用零值
			upCount = 0
		} else if v, ok := item["up_count"].(float64); ok {
			upCount = v
		} else if v, ok := item["up_count"].(int); ok {
			upCount = float64(v)
		} else if v, ok := item["up_count"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				upCount = 0
			} else {
				...
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					...
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["up_count"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: stk_nineturn")
					log.Printf("字段: up_count")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["up_count"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 up_count 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			...
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 up_count 类型，期望 float64/int/string")
		}
		// 处理 down_count 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		...
		if item["down_count"] == nil {
			// 字段值为 null，使用零值
			downCount = 0
		} else if v, ok := item["down_count"].(float64); ok {
			downCount = v
		} else if v, ok := item["down_count"].(int); ok {
			downCount = float64(v)
		} else if v, ok := item["down_count"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				downCount = 0
			} else {
				...
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					...
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["down_count"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: stk_nineturn")
					log.Printf("字段: down_count")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["down_count"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 down_count 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			...
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 down_count 类型，期望 float64/int/string")
		}
