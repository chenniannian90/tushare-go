// Code generated by tushare-gen. DO NOT EDIT.

package stock_feature

import (
	"context"
	"encoding/json"
	"fmt"
	"log"

	"tushare-go/pkg/sdk"
)

// ReportRcRequest 表示 券商盈利预测数据 API 的请求
type ReportRcRequest struct {
	TsCode string `json:"ts_code,omitempty"` // 股票代码
	ReportDate string `json:"report_date,omitempty"` // 报告日期
	StartDate string `json:"start_date,omitempty"` // 报告开始日期
	EndDate string `json:"end_date,omitempty"` // 报告结束日期
}

// ReportRcItem 表示单个 券商盈利预测数据 数据项
type ReportRcItem struct {
	TsCode string `json:"ts_code"` // 股票代码
	Name string `json:"name"` // 股票名称
	ReportDate string `json:"report_date"` // 研报日期
	ReportTitle string `json:"report_title"` // 报告标题
	ReportType string `json:"report_type"` // 报告类型
	Classify string `json:"classify"` // 报告分类
	OrgName string `json:"org_name"` // 机构名称
	AuthorName string `json:"author_name"` // 作者
	Quarter string `json:"quarter"` // 预测报告期
	OpRt float64 `json:"op_rt"` // 预测营业收入（万元）
	OpPr float64 `json:"op_pr"` // 预测营业利润（万元）
	Tp float64 `json:"tp"` // 预测利润总额（万元）
	Np float64 `json:"np"` // 预测净利润（万元）
	Eps float64 `json:"eps"` // 预测每股收益（元）
	Pe float64 `json:"pe"` // 预测市盈率
	Rd float64 `json:"rd"` // 预测股息率
	Roe float64 `json:"roe"` // 预测净资产收益率
	EvEbitda float64 `json:"ev_ebitda"` // 预测EV/EBITDA
	Rating string `json:"rating"` // 卖方评级
	MaxPrice float64 `json:"max_price"` // 预测最高目标价
	MinPrice float64 `json:"min_price"` // 预测最低目标价
	ImpDg string `json:"imp_dg"` // 机构关注度
	CreateTime string `json:"create_time"` // TS数据更新时间
}

// ReportRc 调用 券商盈利预测数据 API
// 获取券商（卖方）每天研报的盈利预测数据，数据从2010年开始，每晚19~22点更新当日数据
// 注意：积分要求等使用限制请参考Tushare官方文档
func ReportRc(ctx context.Context, client *sdk.Client, req *ReportRcRequest) ([]ReportRcItem, error) {
	params := map[string]interface{}{}
	if req.TsCode != "" {
		params["ts_code"] = req.TsCode
	}
	if req.ReportDate != "" {
		params["report_date"] = req.ReportDate
	}
	if req.StartDate != "" {
		params["start_date"] = req.StartDate
	}
	if req.EndDate != "" {
		params["end_date"] = req.EndDate
	}

	fields := []string{"ts_code", "name", "report_date", "report_title", "report_type", "classify", "org_name", "author_name", "quarter", "op_rt", "op_pr", "tp", "np", "eps", "pe", "rd", "roe", "ev_ebitda", "rating", "max_price", "min_price", "imp_dg", "create_time"}

	var result struct {
		Fields []string                 `json:"fields"`
		Items  []map[string]interface{} `json:"items"`
	}

	if err := client.CallAPIFlexible(ctx, "report_rc", params, fields, &result); err != nil {
		return nil, err
	}
	items := make([]ReportRcItem, len(result.Items))
	for i, item := range result.Items {
		// 处理 ts_code 的简单类型
		// 对 string 类型尝试多种转换
		var tsCode string
		if item["ts_code"] == nil {
			// 字段值为 null，使用零值
			tsCode = ""
		} else if v, ok := item["ts_code"].(string); ok {
			tsCode = v
		} else if v, ok := item["ts_code"].(float64); ok {
			tsCode = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ts_code"].(int); ok {
			tsCode = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ts_code"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: ts_code")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ts_code"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ts_code 类型")
		}
		// 处理 name 的简单类型
		// 对 string 类型尝试多种转换
		var name string
		if item["name"] == nil {
			// 字段值为 null，使用零值
			name = ""
		} else if v, ok := item["name"].(string); ok {
			name = v
		} else if v, ok := item["name"].(float64); ok {
			name = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["name"].(int); ok {
			name = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["name"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: name")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["name"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 name 类型")
		}
		// 处理 report_date 的简单类型
		// 对 string 类型尝试多种转换
		var reportDate string
		if item["report_date"] == nil {
			// 字段值为 null，使用零值
			reportDate = ""
		} else if v, ok := item["report_date"].(string); ok {
			reportDate = v
		} else if v, ok := item["report_date"].(float64); ok {
			reportDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["report_date"].(int); ok {
			reportDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["report_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: report_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["report_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 report_date 类型")
		}
		// 处理 report_title 的简单类型
		// 对 string 类型尝试多种转换
		var reportTitle string
		if item["report_title"] == nil {
			// 字段值为 null，使用零值
			reportTitle = ""
		} else if v, ok := item["report_title"].(string); ok {
			reportTitle = v
		} else if v, ok := item["report_title"].(float64); ok {
			reportTitle = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["report_title"].(int); ok {
			reportTitle = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["report_title"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: report_title")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["report_title"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 report_title 类型")
		}
		// 处理 report_type 的简单类型
		// 对 string 类型尝试多种转换
		var reportType string
		if item["report_type"] == nil {
			// 字段值为 null，使用零值
			reportType = ""
		} else if v, ok := item["report_type"].(string); ok {
			reportType = v
		} else if v, ok := item["report_type"].(float64); ok {
			reportType = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["report_type"].(int); ok {
			reportType = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["report_type"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: report_type")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["report_type"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 report_type 类型")
		}
		// 处理 classify 的简单类型
		// 对 string 类型尝试多种转换
		var classify string
		if item["classify"] == nil {
			// 字段值为 null，使用零值
			classify = ""
		} else if v, ok := item["classify"].(string); ok {
			classify = v
		} else if v, ok := item["classify"].(float64); ok {
			classify = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["classify"].(int); ok {
			classify = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["classify"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: classify")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["classify"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 classify 类型")
		}
		// 处理 org_name 的简单类型
		// 对 string 类型尝试多种转换
		var orgName string
		if item["org_name"] == nil {
			// 字段值为 null，使用零值
			orgName = ""
		} else if v, ok := item["org_name"].(string); ok {
			orgName = v
		} else if v, ok := item["org_name"].(float64); ok {
			orgName = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["org_name"].(int); ok {
			orgName = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["org_name"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: org_name")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["org_name"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 org_name 类型")
		}
		// 处理 author_name 的简单类型
		// 对 string 类型尝试多种转换
		var authorName string
		if item["author_name"] == nil {
			// 字段值为 null，使用零值
			authorName = ""
		} else if v, ok := item["author_name"].(string); ok {
			authorName = v
		} else if v, ok := item["author_name"].(float64); ok {
			authorName = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["author_name"].(int); ok {
			authorName = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["author_name"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: author_name")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["author_name"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 author_name 类型")
		}
		// 处理 quarter 的简单类型
		// 对 string 类型尝试多种转换
		var quarter string
		if item["quarter"] == nil {
			// 字段值为 null，使用零值
			quarter = ""
		} else if v, ok := item["quarter"].(string); ok {
			quarter = v
		} else if v, ok := item["quarter"].(float64); ok {
			quarter = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["quarter"].(int); ok {
			quarter = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["quarter"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: quarter")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["quarter"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 quarter 类型")
		}
		// 处理 op_rt 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var opRt float64
		if item["op_rt"] == nil {
			// 字段值为 null，使用零值
			opRt = 0
		} else if v, ok := item["op_rt"].(float64); ok {
			opRt = v
		} else if v, ok := item["op_rt"].(int); ok {
			opRt = float64(v)
		} else if v, ok := item["op_rt"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				opRt = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					opRt = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["op_rt"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: op_rt")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["op_rt"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 op_rt 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["op_rt"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: op_rt")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["op_rt"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 op_rt 类型，期望 float64/int/string")
		}
		// 处理 op_pr 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var opPr float64
		if item["op_pr"] == nil {
			// 字段值为 null，使用零值
			opPr = 0
		} else if v, ok := item["op_pr"].(float64); ok {
			opPr = v
		} else if v, ok := item["op_pr"].(int); ok {
			opPr = float64(v)
		} else if v, ok := item["op_pr"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				opPr = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					opPr = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["op_pr"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: op_pr")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["op_pr"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 op_pr 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["op_pr"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: op_pr")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["op_pr"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 op_pr 类型，期望 float64/int/string")
		}
		// 处理 tp 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var tp float64
		if item["tp"] == nil {
			// 字段值为 null，使用零值
			tp = 0
		} else if v, ok := item["tp"].(float64); ok {
			tp = v
		} else if v, ok := item["tp"].(int); ok {
			tp = float64(v)
		} else if v, ok := item["tp"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				tp = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					tp = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["tp"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: tp")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["tp"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 tp 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["tp"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: tp")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["tp"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 tp 类型，期望 float64/int/string")
		}
		// 处理 np 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var np float64
		if item["np"] == nil {
			// 字段值为 null，使用零值
			np = 0
		} else if v, ok := item["np"].(float64); ok {
			np = v
		} else if v, ok := item["np"].(int); ok {
			np = float64(v)
		} else if v, ok := item["np"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				np = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					np = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["np"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: np")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["np"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 np 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["np"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: np")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["np"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 np 类型，期望 float64/int/string")
		}
		// 处理 eps 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var eps float64
		if item["eps"] == nil {
			// 字段值为 null，使用零值
			eps = 0
		} else if v, ok := item["eps"].(float64); ok {
			eps = v
		} else if v, ok := item["eps"].(int); ok {
			eps = float64(v)
		} else if v, ok := item["eps"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				eps = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					eps = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["eps"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: eps")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["eps"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 eps 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["eps"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: eps")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["eps"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 eps 类型，期望 float64/int/string")
		}
		// 处理 pe 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var pe float64
		if item["pe"] == nil {
			// 字段值为 null，使用零值
			pe = 0
		} else if v, ok := item["pe"].(float64); ok {
			pe = v
		} else if v, ok := item["pe"].(int); ok {
			pe = float64(v)
		} else if v, ok := item["pe"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				pe = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					pe = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["pe"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: pe")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["pe"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 pe 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["pe"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: pe")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["pe"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 pe 类型，期望 float64/int/string")
		}
		// 处理 rd 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var rd float64
		if item["rd"] == nil {
			// 字段值为 null，使用零值
			rd = 0
		} else if v, ok := item["rd"].(float64); ok {
			rd = v
		} else if v, ok := item["rd"].(int); ok {
			rd = float64(v)
		} else if v, ok := item["rd"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				rd = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					rd = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["rd"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: rd")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["rd"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 rd 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["rd"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: rd")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["rd"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 rd 类型，期望 float64/int/string")
		}
		// 处理 roe 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var roe float64
		if item["roe"] == nil {
			// 字段值为 null，使用零值
			roe = 0
		} else if v, ok := item["roe"].(float64); ok {
			roe = v
		} else if v, ok := item["roe"].(int); ok {
			roe = float64(v)
		} else if v, ok := item["roe"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				roe = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					roe = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["roe"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: roe")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["roe"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 roe 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["roe"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: roe")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["roe"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 roe 类型，期望 float64/int/string")
		}
		// 处理 ev_ebitda 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var evEbitda float64
		if item["ev_ebitda"] == nil {
			// 字段值为 null，使用零值
			evEbitda = 0
		} else if v, ok := item["ev_ebitda"].(float64); ok {
			evEbitda = v
		} else if v, ok := item["ev_ebitda"].(int); ok {
			evEbitda = float64(v)
		} else if v, ok := item["ev_ebitda"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				evEbitda = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					evEbitda = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["ev_ebitda"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: ev_ebitda")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["ev_ebitda"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 ev_ebitda 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ev_ebitda"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: ev_ebitda")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ev_ebitda"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ev_ebitda 类型，期望 float64/int/string")
		}
		// 处理 rating 的简单类型
		// 对 string 类型尝试多种转换
		var rating string
		if item["rating"] == nil {
			// 字段值为 null，使用零值
			rating = ""
		} else if v, ok := item["rating"].(string); ok {
			rating = v
		} else if v, ok := item["rating"].(float64); ok {
			rating = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["rating"].(int); ok {
			rating = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["rating"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: rating")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["rating"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 rating 类型")
		}
		// 处理 max_price 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var maxPrice float64
		if item["max_price"] == nil {
			// 字段值为 null，使用零值
			maxPrice = 0
		} else if v, ok := item["max_price"].(float64); ok {
			maxPrice = v
		} else if v, ok := item["max_price"].(int); ok {
			maxPrice = float64(v)
		} else if v, ok := item["max_price"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				maxPrice = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					maxPrice = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["max_price"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: max_price")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["max_price"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 max_price 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["max_price"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: max_price")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["max_price"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 max_price 类型，期望 float64/int/string")
		}
		// 处理 min_price 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var minPrice float64
		if item["min_price"] == nil {
			// 字段值为 null，使用零值
			minPrice = 0
		} else if v, ok := item["min_price"].(float64); ok {
			minPrice = v
		} else if v, ok := item["min_price"].(int); ok {
			minPrice = float64(v)
		} else if v, ok := item["min_price"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				minPrice = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					minPrice = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["min_price"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: report_rc")
					log.Printf("字段: min_price")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["min_price"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 min_price 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["min_price"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: min_price")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["min_price"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 min_price 类型，期望 float64/int/string")
		}
		// 处理 imp_dg 的简单类型
		// 对 string 类型尝试多种转换
		var impDg string
		if item["imp_dg"] == nil {
			// 字段值为 null，使用零值
			impDg = ""
		} else if v, ok := item["imp_dg"].(string); ok {
			impDg = v
		} else if v, ok := item["imp_dg"].(float64); ok {
			impDg = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["imp_dg"].(int); ok {
			impDg = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["imp_dg"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: report_rc")
			log.Printf("字段: imp_dg")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["imp_dg"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 imp_dg 类型")
		}
		// 处理 create_time 的简单类型
		createTime, ok := item["create_time"].(string)
		if !ok {
			return nil, fmt.Errorf("无效的 create_time 类型")
		}
		items[i] = ReportRcItem{
			TsCode: tsCode,
			Name: name,
			ReportDate: reportDate,
			ReportTitle: reportTitle,
			ReportType: reportType,
			Classify: classify,
			OrgName: orgName,
			AuthorName: authorName,
			Quarter: quarter,
			OpRt: opRt,
			OpPr: opPr,
			Tp: tp,
			Np: np,
			Eps: eps,
			Pe: pe,
			Rd: rd,
			Roe: roe,
			EvEbitda: evEbitda,
			Rating: rating,
			MaxPrice: maxPrice,
			MinPrice: minPrice,
			ImpDg: impDg,
			CreateTime: createTime,
		}
	}

	return items, nil
}
