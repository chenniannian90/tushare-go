...
func HkHold(ctx context.Context, client *sdk.Client, req *HkHoldRequest) ([]HkHoldItem, error) {
	...
	for i, item := range result.Items {
		...
		if item["ratio"] == nil {
			// 字段值为 null，使用零值
			ratio = 0
		} else if v, ok := item["ratio"].(float64); ok {
			ratio = v
		} else if v, ok := item["ratio"].(int); ok {
			ratio = float64(v)
		} else if v, ok := item["ratio"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				ratio = 0
			} else {
				...
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					...
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["ratio"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: hk_hold")
					log.Printf("字段: ratio")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["ratio"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 ratio 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			...
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ratio 类型，期望 float64/int/string")
		}
		// 处理 exchange 的简单类型
		// 对 string 类型尝试多种转换
		var exchange string
