...
func BoWeekly(ctx context.Context, client *sdk.Client, req *BoWeeklyRequest) ([]BoWeeklyItem, error) {
	...
	fields := []string{"date", "name", "avg_price", "week_amount", "total", "list_day", "p_pc", "wom_index", "up_ratio", "rank"}
	...
	for i, item := range result.Items {
		...
		// 处理 week_amount 的简单类型
		weekAmount, ok := item["week_amount"].(float64)
		if !ok {
			return nil, fmt.Errorf("无效的 week_amount 类型")
		}
		// 处理 total 的简单类型
