// Code generated by tushare-gen. DO NOT EDIT.

package fund

import (
	"context"
	"encoding/json"
	"fmt"
	"log"

	"tushare-go/pkg/sdk"
)

// FundDivRequest 表示 基金分红 API 的请求
type FundDivRequest struct {
	AnnDate string `json:"ann_date,omitempty"` // 公告日（以下参数四选一）
	ExDate string `json:"ex_date,omitempty"` // 除息日
	PayDate string `json:"pay_date,omitempty"` // 派息日
	TsCode string `json:"ts_code,omitempty"` // 基金代码
}

// FundDivItem 表示单个 基金分红 数据项
type FundDivItem struct {
	TsCode string `json:"ts_code"` // TS代码
	AnnDate string `json:"ann_date"` // 公告日期
	ImpAnndate string `json:"imp_anndate"` // 分红实施公告日
	BaseDate string `json:"base_date"` // 分配收益基准日
	DivProc string `json:"div_proc"` // 方案进度
	RecordDate string `json:"record_date"` // 权益登记日
	ExDate string `json:"ex_date"` // 除息日
	PayDate string `json:"pay_date"` // 派息日
	EarpayDate string `json:"earpay_date"` // 收益支付日
	NetExDate string `json:"net_ex_date"` // 净值除权日
	DivCash float64 `json:"div_cash"` // 每股派息(元)
	BaseUnit float64 `json:"base_unit"` // 基准基金份额(万份)
	EarDistr float64 `json:"ear_distr"` // 可分配收益(元)
	EarAmount float64 `json:"ear_amount"` // 收益分配金额(元)
	AccountDate string `json:"account_date"` // 红利再投资到账日
	BaseYear string `json:"base_year"` // 份额基准年度
}

// FundDiv 调用 基金分红 API
// 获取公募基金分红数据
// 注意：积分要求等使用限制请参考Tushare官方文档
func FundDiv(ctx context.Context, client *sdk.Client, req *FundDivRequest) ([]FundDivItem, error) {
	params := map[string]interface{}{}
	if req.AnnDate != "" {
		params["ann_date"] = req.AnnDate
	}
	if req.ExDate != "" {
		params["ex_date"] = req.ExDate
	}
	if req.PayDate != "" {
		params["pay_date"] = req.PayDate
	}
	if req.TsCode != "" {
		params["ts_code"] = req.TsCode
	}

	fields := []string{"ts_code", "ann_date", "imp_anndate", "base_date", "div_proc", "record_date", "ex_date", "pay_date", "earpay_date", "net_ex_date", "div_cash", "base_unit", "ear_distr", "ear_amount", "account_date", "base_year"}

	var result struct {
		Fields []string                 `json:"fields"`
		Items  []map[string]interface{} `json:"items"`
	}

	if err := client.CallAPIFlexible(ctx, "fund_div", params, fields, &result); err != nil {
		return nil, err
	}
	items := make([]FundDivItem, len(result.Items))
	for i, item := range result.Items {
		// 处理 ts_code 的简单类型
		// 对 string 类型尝试多种转换
		var tsCode string
		if item["ts_code"] == nil {
			// 字段值为 null，使用零值
			tsCode = ""
		} else if v, ok := item["ts_code"].(string); ok {
			tsCode = v
		} else if v, ok := item["ts_code"].(float64); ok {
			tsCode = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ts_code"].(int); ok {
			tsCode = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ts_code"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: ts_code")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ts_code"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ts_code 类型")
		}
		// 处理 ann_date 的简单类型
		// 对 string 类型尝试多种转换
		var annDate string
		if item["ann_date"] == nil {
			// 字段值为 null，使用零值
			annDate = ""
		} else if v, ok := item["ann_date"].(string); ok {
			annDate = v
		} else if v, ok := item["ann_date"].(float64); ok {
			annDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ann_date"].(int); ok {
			annDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ann_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: ann_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ann_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ann_date 类型")
		}
		// 处理 imp_anndate 的简单类型
		// 对 string 类型尝试多种转换
		var impAnndate string
		if item["imp_anndate"] == nil {
			// 字段值为 null，使用零值
			impAnndate = ""
		} else if v, ok := item["imp_anndate"].(string); ok {
			impAnndate = v
		} else if v, ok := item["imp_anndate"].(float64); ok {
			impAnndate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["imp_anndate"].(int); ok {
			impAnndate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["imp_anndate"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: imp_anndate")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["imp_anndate"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 imp_anndate 类型")
		}
		// 处理 base_date 的简单类型
		// 对 string 类型尝试多种转换
		var baseDate string
		if item["base_date"] == nil {
			// 字段值为 null，使用零值
			baseDate = ""
		} else if v, ok := item["base_date"].(string); ok {
			baseDate = v
		} else if v, ok := item["base_date"].(float64); ok {
			baseDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["base_date"].(int); ok {
			baseDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["base_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: base_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["base_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 base_date 类型")
		}
		// 处理 div_proc 的简单类型
		// 对 string 类型尝试多种转换
		var divProc string
		if item["div_proc"] == nil {
			// 字段值为 null，使用零值
			divProc = ""
		} else if v, ok := item["div_proc"].(string); ok {
			divProc = v
		} else if v, ok := item["div_proc"].(float64); ok {
			divProc = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["div_proc"].(int); ok {
			divProc = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["div_proc"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: div_proc")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["div_proc"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 div_proc 类型")
		}
		// 处理 record_date 的简单类型
		// 对 string 类型尝试多种转换
		var recordDate string
		if item["record_date"] == nil {
			// 字段值为 null，使用零值
			recordDate = ""
		} else if v, ok := item["record_date"].(string); ok {
			recordDate = v
		} else if v, ok := item["record_date"].(float64); ok {
			recordDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["record_date"].(int); ok {
			recordDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["record_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: record_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["record_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 record_date 类型")
		}
		// 处理 ex_date 的简单类型
		// 对 string 类型尝试多种转换
		var exDate string
		if item["ex_date"] == nil {
			// 字段值为 null，使用零值
			exDate = ""
		} else if v, ok := item["ex_date"].(string); ok {
			exDate = v
		} else if v, ok := item["ex_date"].(float64); ok {
			exDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ex_date"].(int); ok {
			exDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ex_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: ex_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ex_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ex_date 类型")
		}
		// 处理 pay_date 的简单类型
		// 对 string 类型尝试多种转换
		var payDate string
		if item["pay_date"] == nil {
			// 字段值为 null，使用零值
			payDate = ""
		} else if v, ok := item["pay_date"].(string); ok {
			payDate = v
		} else if v, ok := item["pay_date"].(float64); ok {
			payDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["pay_date"].(int); ok {
			payDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["pay_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: pay_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["pay_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 pay_date 类型")
		}
		// 处理 earpay_date 的简单类型
		// 对 string 类型尝试多种转换
		var earpayDate string
		if item["earpay_date"] == nil {
			// 字段值为 null，使用零值
			earpayDate = ""
		} else if v, ok := item["earpay_date"].(string); ok {
			earpayDate = v
		} else if v, ok := item["earpay_date"].(float64); ok {
			earpayDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["earpay_date"].(int); ok {
			earpayDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["earpay_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: earpay_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["earpay_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 earpay_date 类型")
		}
		// 处理 net_ex_date 的简单类型
		// 对 string 类型尝试多种转换
		var netExDate string
		if item["net_ex_date"] == nil {
			// 字段值为 null，使用零值
			netExDate = ""
		} else if v, ok := item["net_ex_date"].(string); ok {
			netExDate = v
		} else if v, ok := item["net_ex_date"].(float64); ok {
			netExDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["net_ex_date"].(int); ok {
			netExDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["net_ex_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: net_ex_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["net_ex_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 net_ex_date 类型")
		}
		// 处理 div_cash 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var divCash float64
		if item["div_cash"] == nil {
			// 字段值为 null，使用零值
			divCash = 0
		} else if v, ok := item["div_cash"].(float64); ok {
			divCash = v
		} else if v, ok := item["div_cash"].(int); ok {
			divCash = float64(v)
		} else if v, ok := item["div_cash"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				divCash = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					divCash = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["div_cash"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: fund_div")
					log.Printf("字段: div_cash")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["div_cash"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 div_cash 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["div_cash"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: div_cash")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["div_cash"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 div_cash 类型，期望 float64/int/string")
		}
		// 处理 base_unit 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var baseUnit float64
		if item["base_unit"] == nil {
			// 字段值为 null，使用零值
			baseUnit = 0
		} else if v, ok := item["base_unit"].(float64); ok {
			baseUnit = v
		} else if v, ok := item["base_unit"].(int); ok {
			baseUnit = float64(v)
		} else if v, ok := item["base_unit"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				baseUnit = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					baseUnit = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["base_unit"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: fund_div")
					log.Printf("字段: base_unit")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["base_unit"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 base_unit 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["base_unit"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: base_unit")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["base_unit"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 base_unit 类型，期望 float64/int/string")
		}
		// 处理 ear_distr 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var earDistr float64
		if item["ear_distr"] == nil {
			// 字段值为 null，使用零值
			earDistr = 0
		} else if v, ok := item["ear_distr"].(float64); ok {
			earDistr = v
		} else if v, ok := item["ear_distr"].(int); ok {
			earDistr = float64(v)
		} else if v, ok := item["ear_distr"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				earDistr = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					earDistr = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["ear_distr"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: fund_div")
					log.Printf("字段: ear_distr")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["ear_distr"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 ear_distr 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ear_distr"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: ear_distr")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ear_distr"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ear_distr 类型，期望 float64/int/string")
		}
		// 处理 ear_amount 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var earAmount float64
		if item["ear_amount"] == nil {
			// 字段值为 null，使用零值
			earAmount = 0
		} else if v, ok := item["ear_amount"].(float64); ok {
			earAmount = v
		} else if v, ok := item["ear_amount"].(int); ok {
			earAmount = float64(v)
		} else if v, ok := item["ear_amount"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				earAmount = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					earAmount = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["ear_amount"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: fund_div")
					log.Printf("字段: ear_amount")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["ear_amount"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 ear_amount 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ear_amount"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: ear_amount")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ear_amount"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ear_amount 类型，期望 float64/int/string")
		}
		// 处理 account_date 的简单类型
		// 对 string 类型尝试多种转换
		var accountDate string
		if item["account_date"] == nil {
			// 字段值为 null，使用零值
			accountDate = ""
		} else if v, ok := item["account_date"].(string); ok {
			accountDate = v
		} else if v, ok := item["account_date"].(float64); ok {
			accountDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["account_date"].(int); ok {
			accountDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["account_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: account_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["account_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 account_date 类型")
		}
		// 处理 base_year 的简单类型
		// 对 string 类型尝试多种转换
		var baseYear string
		if item["base_year"] == nil {
			// 字段值为 null，使用零值
			baseYear = ""
		} else if v, ok := item["base_year"].(string); ok {
			baseYear = v
		} else if v, ok := item["base_year"].(float64); ok {
			baseYear = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["base_year"].(int); ok {
			baseYear = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["base_year"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fund_div")
			log.Printf("字段: base_year")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["base_year"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 base_year 类型")
		}
		items[i] = FundDivItem{
			TsCode: tsCode,
			AnnDate: annDate,
			ImpAnndate: impAnndate,
			BaseDate: baseDate,
			DivProc: divProc,
			RecordDate: recordDate,
			ExDate: exDate,
			PayDate: payDate,
			EarpayDate: earpayDate,
			NetExDate: netExDate,
			DivCash: divCash,
			BaseUnit: baseUnit,
			EarDistr: earDistr,
			EarAmount: earAmount,
			AccountDate: accountDate,
			BaseYear: baseYear,
		}
	}

	return items, nil
}
