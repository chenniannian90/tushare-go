...
func TestStockBasicRequest_Validate(t *testing.T) {
	tests := []struct {
		name    string
		...
		{
			name: "无效请求 - list_status",
			req: &StockBasicRequest{
				ListStatus: "INVALID",
			},
			wantErr: true,
			errMsg:  "list_status 必须为 L、D、P、del 之一",
		},
		{
			name: "无效请求 - exchange",
			req: &StockBasicRequest{
			...
			wantErr: true,
			errMsg:  "exchange 必须为 SSE、SZSE、CZCE、SHFE、INE、BJE 之一",
		},
	}
