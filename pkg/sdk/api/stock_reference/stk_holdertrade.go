// Code generated by tushare-gen. DO NOT EDIT.

package stock_reference

import (
	"context"
	"encoding/json"
	"fmt"
	"log"

	"tushare-go/pkg/sdk"
)

// StkHoldertradeRequest 表示 股东增减持 API 的请求
type StkHoldertradeRequest struct {
	TsCode string `json:"ts_code,omitempty"` // TS股票代码
	AnnDate string `json:"ann_date,omitempty"` // 公告日期
	StartDate string `json:"start_date,omitempty"` // 公告开始日期
	EndDate string `json:"end_date,omitempty"` // 公告结束日期
	TradeType string `json:"trade_type,omitempty"` // 交易类型IN增持DE减持
	HolderType string `json:"holder_type,omitempty"` // 股东类型C公司P个人G高管
}

// StkHoldertradeItem 表示单个 股东增减持 数据项
type StkHoldertradeItem struct {
	TsCode string `json:"ts_code"` // TS代码
	AnnDate string `json:"ann_date"` // 公告日期
	HolderName string `json:"holder_name"` // 股东名称
	HolderType string `json:"holder_type"` // 股东类型G高管P个人C公司
	InDe string `json:"in_de"` // 类型IN增持DE减持
	ChangeVol float64 `json:"change_vol"` // 变动数量
	ChangeRatio float64 `json:"change_ratio"` // 占流通比例（%）
	AfterShare float64 `json:"after_share"` // 变动后持股
	AfterRatio float64 `json:"after_ratio"` // 变动后占流通比例（%）
	AvgPrice float64 `json:"avg_price"` // 平均价格
	TotalShare float64 `json:"total_share"` // 持股总数
	BeginDate string `json:"begin_date"` // 增减持开始日期
	CloseDate string `json:"close_date"` // 增减持结束日期
}

// StkHoldertrade 调用 股东增减持 API
// 获取上市公司增减持数据，了解重要股东近期及历史上的股份增减变化
// 注意：积分要求等使用限制请参考Tushare官方文档
func StkHoldertrade(ctx context.Context, client *sdk.Client, req *StkHoldertradeRequest) ([]StkHoldertradeItem, error) {
	params := map[string]interface{}{}
	if req.TsCode != "" {
		params["ts_code"] = req.TsCode
	}
	if req.AnnDate != "" {
		params["ann_date"] = req.AnnDate
	}
	if req.StartDate != "" {
		params["start_date"] = req.StartDate
	}
	if req.EndDate != "" {
		params["end_date"] = req.EndDate
	}
	if req.TradeType != "" {
		params["trade_type"] = req.TradeType
	}
	if req.HolderType != "" {
		params["holder_type"] = req.HolderType
	}

	fields := []string{"ts_code", "ann_date", "holder_name", "holder_type", "in_de", "change_vol", "change_ratio", "after_share", "after_ratio", "avg_price", "total_share", "begin_date", "close_date"}

	var result struct {
		Fields []string                 `json:"fields"`
		Items  []map[string]interface{} `json:"items"`
	}

	if err := client.CallAPIFlexible(ctx, "stk_holdertrade", params, fields, &result); err != nil {
		return nil, err
	}
	items := make([]StkHoldertradeItem, len(result.Items))
	for i, item := range result.Items {
		// 处理 ts_code 的简单类型
		// 对 string 类型尝试多种转换
		var tsCode string
		if item["ts_code"] == nil {
			// 字段值为 null，使用零值
			tsCode = ""
		} else if v, ok := item["ts_code"].(string); ok {
			tsCode = v
		} else if v, ok := item["ts_code"].(float64); ok {
			tsCode = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ts_code"].(int); ok {
			tsCode = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ts_code"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: ts_code")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ts_code"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ts_code 类型")
		}
		// 处理 ann_date 的简单类型
		// 对 string 类型尝试多种转换
		var annDate string
		if item["ann_date"] == nil {
			// 字段值为 null，使用零值
			annDate = ""
		} else if v, ok := item["ann_date"].(string); ok {
			annDate = v
		} else if v, ok := item["ann_date"].(float64); ok {
			annDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ann_date"].(int); ok {
			annDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ann_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: ann_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ann_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ann_date 类型")
		}
		// 处理 holder_name 的简单类型
		// 对 string 类型尝试多种转换
		var holderName string
		if item["holder_name"] == nil {
			// 字段值为 null，使用零值
			holderName = ""
		} else if v, ok := item["holder_name"].(string); ok {
			holderName = v
		} else if v, ok := item["holder_name"].(float64); ok {
			holderName = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["holder_name"].(int); ok {
			holderName = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["holder_name"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: holder_name")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["holder_name"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 holder_name 类型")
		}
		// 处理 holder_type 的简单类型
		// 对 string 类型尝试多种转换
		var holderType string
		if item["holder_type"] == nil {
			// 字段值为 null，使用零值
			holderType = ""
		} else if v, ok := item["holder_type"].(string); ok {
			holderType = v
		} else if v, ok := item["holder_type"].(float64); ok {
			holderType = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["holder_type"].(int); ok {
			holderType = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["holder_type"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: holder_type")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["holder_type"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 holder_type 类型")
		}
		// 处理 in_de 的简单类型
		// 对 string 类型尝试多种转换
		var inDe string
		if item["in_de"] == nil {
			// 字段值为 null，使用零值
			inDe = ""
		} else if v, ok := item["in_de"].(string); ok {
			inDe = v
		} else if v, ok := item["in_de"].(float64); ok {
			inDe = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["in_de"].(int); ok {
			inDe = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["in_de"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: in_de")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["in_de"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 in_de 类型")
		}
		// 处理 change_vol 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var changeVol float64
		if item["change_vol"] == nil {
			// 字段值为 null，使用零值
			changeVol = 0
		} else if v, ok := item["change_vol"].(float64); ok {
			changeVol = v
		} else if v, ok := item["change_vol"].(int); ok {
			changeVol = float64(v)
		} else if v, ok := item["change_vol"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				changeVol = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					changeVol = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["change_vol"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: stk_holdertrade")
					log.Printf("字段: change_vol")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["change_vol"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 change_vol 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["change_vol"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: change_vol")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["change_vol"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 change_vol 类型，期望 float64/int/string")
		}
		// 处理 change_ratio 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var changeRatio float64
		if item["change_ratio"] == nil {
			// 字段值为 null，使用零值
			changeRatio = 0
		} else if v, ok := item["change_ratio"].(float64); ok {
			changeRatio = v
		} else if v, ok := item["change_ratio"].(int); ok {
			changeRatio = float64(v)
		} else if v, ok := item["change_ratio"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				changeRatio = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					changeRatio = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["change_ratio"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: stk_holdertrade")
					log.Printf("字段: change_ratio")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["change_ratio"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 change_ratio 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["change_ratio"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: change_ratio")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["change_ratio"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 change_ratio 类型，期望 float64/int/string")
		}
		// 处理 after_share 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var afterShare float64
		if item["after_share"] == nil {
			// 字段值为 null，使用零值
			afterShare = 0
		} else if v, ok := item["after_share"].(float64); ok {
			afterShare = v
		} else if v, ok := item["after_share"].(int); ok {
			afterShare = float64(v)
		} else if v, ok := item["after_share"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				afterShare = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					afterShare = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["after_share"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: stk_holdertrade")
					log.Printf("字段: after_share")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["after_share"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 after_share 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["after_share"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: after_share")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["after_share"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 after_share 类型，期望 float64/int/string")
		}
		// 处理 after_ratio 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var afterRatio float64
		if item["after_ratio"] == nil {
			// 字段值为 null，使用零值
			afterRatio = 0
		} else if v, ok := item["after_ratio"].(float64); ok {
			afterRatio = v
		} else if v, ok := item["after_ratio"].(int); ok {
			afterRatio = float64(v)
		} else if v, ok := item["after_ratio"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				afterRatio = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					afterRatio = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["after_ratio"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: stk_holdertrade")
					log.Printf("字段: after_ratio")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["after_ratio"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 after_ratio 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["after_ratio"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: after_ratio")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["after_ratio"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 after_ratio 类型，期望 float64/int/string")
		}
		// 处理 avg_price 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var avgPrice float64
		if item["avg_price"] == nil {
			// 字段值为 null，使用零值
			avgPrice = 0
		} else if v, ok := item["avg_price"].(float64); ok {
			avgPrice = v
		} else if v, ok := item["avg_price"].(int); ok {
			avgPrice = float64(v)
		} else if v, ok := item["avg_price"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				avgPrice = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					avgPrice = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["avg_price"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: stk_holdertrade")
					log.Printf("字段: avg_price")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["avg_price"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 avg_price 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["avg_price"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: avg_price")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["avg_price"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 avg_price 类型，期望 float64/int/string")
		}
		// 处理 total_share 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var totalShare float64
		if item["total_share"] == nil {
			// 字段值为 null，使用零值
			totalShare = 0
		} else if v, ok := item["total_share"].(float64); ok {
			totalShare = v
		} else if v, ok := item["total_share"].(int); ok {
			totalShare = float64(v)
		} else if v, ok := item["total_share"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				totalShare = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					totalShare = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["total_share"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: stk_holdertrade")
					log.Printf("字段: total_share")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["total_share"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 total_share 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["total_share"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: total_share")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["total_share"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 total_share 类型，期望 float64/int/string")
		}
		// 处理 begin_date 的简单类型
		// 对 string 类型尝试多种转换
		var beginDate string
		if item["begin_date"] == nil {
			// 字段值为 null，使用零值
			beginDate = ""
		} else if v, ok := item["begin_date"].(string); ok {
			beginDate = v
		} else if v, ok := item["begin_date"].(float64); ok {
			beginDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["begin_date"].(int); ok {
			beginDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["begin_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: begin_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["begin_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 begin_date 类型")
		}
		// 处理 close_date 的简单类型
		// 对 string 类型尝试多种转换
		var closeDate string
		if item["close_date"] == nil {
			// 字段值为 null，使用零值
			closeDate = ""
		} else if v, ok := item["close_date"].(string); ok {
			closeDate = v
		} else if v, ok := item["close_date"].(float64); ok {
			closeDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["close_date"].(int); ok {
			closeDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["close_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: stk_holdertrade")
			log.Printf("字段: close_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["close_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 close_date 类型")
		}
		items[i] = StkHoldertradeItem{
			TsCode: tsCode,
			AnnDate: annDate,
			HolderName: holderName,
			HolderType: holderType,
			InDe: inDe,
			ChangeVol: changeVol,
			ChangeRatio: changeRatio,
			AfterShare: afterShare,
			AfterRatio: afterRatio,
			AvgPrice: avgPrice,
			TotalShare: totalShare,
			BeginDate: beginDate,
			CloseDate: closeDate,
		}
	}

	return items, nil
}
