...
func KplConceptCons(ctx context.Context, client *sdk.Client, req *KplConceptConsRequest) ([]KplConceptConsItem, error) {
	...
	for i, item := range result.Items {
		...
		// 对 string 类型尝试多种转换
		var conName string
		if item["con_name"] == nil {
			// 字段值为 null，使用零值
			conName = ""
		} else if v, ok := item["con_name"].(string); ok {
			conName = v
		...
		// 对 string 类型尝试多种转换
		var conCode string
		if item["con_code"] == nil {
			// 字段值为 null，使用零值
			conCode = ""
		} else if v, ok := item["con_code"].(string); ok {
			conCode = v
		...
		if item["hot_num"] == nil {
			// 字段值为 null，使用零值
			hotNum = 0
		} else if v, ok := item["hot_num"].(float64); ok {
			hotNum = int(v)
		} else if v, ok := item["hot_num"].(int); ok {
			hotNum = v
		} else {
			return nil, fmt.Errorf("无效的 hot_num 类型，期望 int 或 float64")
		}
