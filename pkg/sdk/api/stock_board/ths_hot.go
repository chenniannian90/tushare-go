// Code generated by tushare-gen. DO NOT EDIT.

package stock_board

import (
	"context"
	"encoding/json"
	"fmt"
	"log"

	"tushare-go/pkg/sdk"
)

// ThsHotRequest 表示 同花顺App热榜数 API 的请求
type ThsHotRequest struct {
	TradeDate string `json:"trade_date,omitempty"` // 交易日期
	TsCode string `json:"ts_code,omitempty"` // TS代码
	Market string `json:"market,omitempty"` // 热榜类型(热股、ETF、可转债、行业板块、概念板块、期货、港股、热基、美股)
	IsNew string `json:"is_new,omitempty"` // 是否最新（默认Y，如果为N则为盘中和盘后阶段采集，具体时间可参考rank_time字段，状态N每小时更新一次，状态Y更新时间为22：30）
}

// ThsHotItem 表示单个 同花顺App热榜数 数据项
type ThsHotItem struct {
	TradeDate string `json:"trade_date"` // 交易日期
	DataType string `json:"data_type"` // 数据类型
	TsCode string `json:"ts_code"` // 股票代码
	TsName string `json:"ts_name"` // 股票名称
	Rank int `json:"rank"` // 排行
	PctChange float64 `json:"pct_change"` // 涨跌幅%
	CurrentPrice float64 `json:"current_price"` // 当前价格
	Concept string `json:"concept"` // 标签
	RankReason string `json:"rank_reason"` // 上榜解读
	Hot float64 `json:"hot"` // 热度值
	RankTime string `json:"rank_time"` // 排行榜获取时间
}

// ThsHot 调用 同花顺App热榜数 API
// 获取同花顺App热榜数据，包括热股、概念板块、ETF、可转债、港美股等等，每日盘中提取4次，收盘后4次，最晚22点提取一次。
// 注意：积分要求等使用限制请参考Tushare官方文档
func ThsHot(ctx context.Context, client *sdk.Client, req *ThsHotRequest) ([]ThsHotItem, error) {
	params := map[string]interface{}{}
	if req.TradeDate != "" {
		params["trade_date"] = req.TradeDate
	}
	if req.TsCode != "" {
		params["ts_code"] = req.TsCode
	}
	if req.Market != "" {
		params["market"] = req.Market
	}
	if req.IsNew != "" {
		params["is_new"] = req.IsNew
	}

	fields := []string{"trade_date", "data_type", "ts_code", "ts_name", "rank", "pct_change", "current_price", "concept", "rank_reason", "hot", "rank_time"}

	var result struct {
		Fields []string                 `json:"fields"`
		Items  []map[string]interface{} `json:"items"`
	}

	if err := client.CallAPIFlexible(ctx, "ths_hot", params, fields, &result); err != nil {
		return nil, err
	}
	items := make([]ThsHotItem, len(result.Items))
	for i, item := range result.Items {
		// 处理 trade_date 的简单类型
		// 对 string 类型尝试多种转换
		var tradeDate string
		if item["trade_date"] == nil {
			// 字段值为 null，使用零值
			tradeDate = ""
		} else if v, ok := item["trade_date"].(string); ok {
			tradeDate = v
		} else if v, ok := item["trade_date"].(float64); ok {
			tradeDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["trade_date"].(int); ok {
			tradeDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["trade_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: trade_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["trade_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 trade_date 类型")
		}
		// 处理 data_type 的简单类型
		// 对 string 类型尝试多种转换
		var dataType string
		if item["data_type"] == nil {
			// 字段值为 null，使用零值
			dataType = ""
		} else if v, ok := item["data_type"].(string); ok {
			dataType = v
		} else if v, ok := item["data_type"].(float64); ok {
			dataType = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["data_type"].(int); ok {
			dataType = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["data_type"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: data_type")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["data_type"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 data_type 类型")
		}
		// 处理 ts_code 的简单类型
		// 对 string 类型尝试多种转换
		var tsCode string
		if item["ts_code"] == nil {
			// 字段值为 null，使用零值
			tsCode = ""
		} else if v, ok := item["ts_code"].(string); ok {
			tsCode = v
		} else if v, ok := item["ts_code"].(float64); ok {
			tsCode = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ts_code"].(int); ok {
			tsCode = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ts_code"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: ts_code")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ts_code"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ts_code 类型")
		}
		// 处理 ts_name 的简单类型
		// 对 string 类型尝试多种转换
		var tsName string
		if item["ts_name"] == nil {
			// 字段值为 null，使用零值
			tsName = ""
		} else if v, ok := item["ts_name"].(string); ok {
			tsName = v
		} else if v, ok := item["ts_name"].(float64); ok {
			tsName = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ts_name"].(int); ok {
			tsName = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ts_name"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: ts_name")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ts_name"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ts_name 类型")
		}
		// 处理 rank 的简单类型
		// 处理 int 类型 - JSON 数字解析为 float64，需要转换
		var rank int
		if item["rank"] == nil {
			// 字段值为 null，使用零值
			rank = 0
		} else if v, ok := item["rank"].(float64); ok {
			rank = int(v)
		} else if v, ok := item["rank"].(int); ok {
			rank = v
		} else {
			return nil, fmt.Errorf("无效的 rank 类型，期望 int 或 float64")
		}
		// 处理 pct_change 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var pctChange float64
		if item["pct_change"] == nil {
			// 字段值为 null，使用零值
			pctChange = 0
		} else if v, ok := item["pct_change"].(float64); ok {
			pctChange = v
		} else if v, ok := item["pct_change"].(int); ok {
			pctChange = float64(v)
		} else if v, ok := item["pct_change"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				pctChange = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					pctChange = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["pct_change"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: ths_hot")
					log.Printf("字段: pct_change")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["pct_change"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 pct_change 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["pct_change"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: pct_change")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["pct_change"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 pct_change 类型，期望 float64/int/string")
		}
		// 处理 current_price 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var currentPrice float64
		if item["current_price"] == nil {
			// 字段值为 null，使用零值
			currentPrice = 0
		} else if v, ok := item["current_price"].(float64); ok {
			currentPrice = v
		} else if v, ok := item["current_price"].(int); ok {
			currentPrice = float64(v)
		} else if v, ok := item["current_price"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				currentPrice = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					currentPrice = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["current_price"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: ths_hot")
					log.Printf("字段: current_price")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["current_price"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 current_price 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["current_price"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: current_price")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["current_price"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 current_price 类型，期望 float64/int/string")
		}
		// 处理 concept 的简单类型
		// 对 string 类型尝试多种转换
		var concept string
		if item["concept"] == nil {
			// 字段值为 null，使用零值
			concept = ""
		} else if v, ok := item["concept"].(string); ok {
			concept = v
		} else if v, ok := item["concept"].(float64); ok {
			concept = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["concept"].(int); ok {
			concept = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["concept"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: concept")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["concept"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 concept 类型")
		}
		// 处理 rank_reason 的简单类型
		// 对 string 类型尝试多种转换
		var rankReason string
		if item["rank_reason"] == nil {
			// 字段值为 null，使用零值
			rankReason = ""
		} else if v, ok := item["rank_reason"].(string); ok {
			rankReason = v
		} else if v, ok := item["rank_reason"].(float64); ok {
			rankReason = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["rank_reason"].(int); ok {
			rankReason = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["rank_reason"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: rank_reason")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["rank_reason"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 rank_reason 类型")
		}
		// 处理 hot 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var hot float64
		if item["hot"] == nil {
			// 字段值为 null，使用零值
			hot = 0
		} else if v, ok := item["hot"].(float64); ok {
			hot = v
		} else if v, ok := item["hot"].(int); ok {
			hot = float64(v)
		} else if v, ok := item["hot"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				hot = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					hot = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["hot"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: ths_hot")
					log.Printf("字段: hot")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["hot"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 hot 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["hot"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: hot")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["hot"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 hot 类型，期望 float64/int/string")
		}
		// 处理 rank_time 的简单类型
		// 对 string 类型尝试多种转换
		var rankTime string
		if item["rank_time"] == nil {
			// 字段值为 null，使用零值
			rankTime = ""
		} else if v, ok := item["rank_time"].(string); ok {
			rankTime = v
		} else if v, ok := item["rank_time"].(float64); ok {
			rankTime = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["rank_time"].(int); ok {
			rankTime = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["rank_time"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: ths_hot")
			log.Printf("字段: rank_time")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["rank_time"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 rank_time 类型")
		}
		items[i] = ThsHotItem{
			TradeDate: tradeDate,
			DataType: dataType,
			TsCode: tsCode,
			TsName: tsName,
			Rank: rank,
			PctChange: pctChange,
			CurrentPrice: currentPrice,
			Concept: concept,
			RankReason: rankReason,
			Hot: hot,
			RankTime: rankTime,
		}
	}

	return items, nil
}
