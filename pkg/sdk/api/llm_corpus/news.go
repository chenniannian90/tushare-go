...
func News(ctx context.Context, client *sdk.Client, req *NewsRequest) ([]NewsItem, error) {
	...
	for i, item := range result.Items {
		...
		if v, ok := item["datetime"].(string); ok {
			...
			fieldJSON, _ := json.Marshal(item["datetime"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: news")
			log.Printf("字段: datetime")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["datetime"])
		...
		if v, ok := item["channels"].(string); ok {
			...
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["channels"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: news")
			log.Printf("字段: channels")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["channels"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
