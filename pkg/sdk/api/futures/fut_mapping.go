// Code generated by tushare-gen. DO NOT EDIT.

package futures

import (
	"context"
	"encoding/json"
	"fmt"
	"log"

	"tushare-go/pkg/sdk"
)

// FutMappingRequest 表示 期货主力与连续合约 API 的请求
type FutMappingRequest struct {
	TsCode string `json:"ts_code,omitempty"` // 合约代码
	TradeDate string `json:"trade_date,omitempty"` // 交易日期(YYYYMMDD格式，下同)
	StartDate string `json:"start_date,omitempty"` // 开始日期
	EndDate string `json:"end_date,omitempty"` // 结束日期
}

// FutMappingItem 表示单个 期货主力与连续合约 数据项
type FutMappingItem struct {
	TsCode string `json:"ts_code"` // 连续合约代码
	TradeDate string `json:"trade_date"` // 起始日期
	MappingTsCode string `json:"mapping_ts_code"` // 期货合约代码
}

// FutMapping 调用 期货主力与连续合约 API
// 获取期货主力（或连续）合约与月合约映射数据
// 注意：积分要求等使用限制请参考Tushare官方文档
func FutMapping(ctx context.Context, client *sdk.Client, req *FutMappingRequest) ([]FutMappingItem, error) {
	params := map[string]interface{}{}
	if req.TsCode != "" {
		params["ts_code"] = req.TsCode
	}
	if req.TradeDate != "" {
		params["trade_date"] = req.TradeDate
	}
	if req.StartDate != "" {
		params["start_date"] = req.StartDate
	}
	if req.EndDate != "" {
		params["end_date"] = req.EndDate
	}

	fields := []string{"ts_code", "trade_date", "mapping_ts_code"}

	var result struct {
		Fields []string                 `json:"fields"`
		Items  []map[string]interface{} `json:"items"`
	}

	if err := client.CallAPIFlexible(ctx, "fut_mapping", params, fields, &result); err != nil {
		return nil, err
	}
	items := make([]FutMappingItem, len(result.Items))
	for i, item := range result.Items {
		// 处理 ts_code 的简单类型
		// 对 string 类型尝试多种转换
		var tsCode string
		if item["ts_code"] == nil {
			// 字段值为 null，使用零值
			tsCode = ""
		} else if v, ok := item["ts_code"].(string); ok {
			tsCode = v
		} else if v, ok := item["ts_code"].(float64); ok {
			tsCode = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ts_code"].(int); ok {
			tsCode = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ts_code"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fut_mapping")
			log.Printf("字段: ts_code")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ts_code"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ts_code 类型")
		}
		// 处理 trade_date 的简单类型
		// 对 string 类型尝试多种转换
		var tradeDate string
		if item["trade_date"] == nil {
			// 字段值为 null，使用零值
			tradeDate = ""
		} else if v, ok := item["trade_date"].(string); ok {
			tradeDate = v
		} else if v, ok := item["trade_date"].(float64); ok {
			tradeDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["trade_date"].(int); ok {
			tradeDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["trade_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fut_mapping")
			log.Printf("字段: trade_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["trade_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 trade_date 类型")
		}
		// 处理 mapping_ts_code 的简单类型
		// 对 string 类型尝试多种转换
		var mappingTsCode string
		if item["mapping_ts_code"] == nil {
			// 字段值为 null，使用零值
			mappingTsCode = ""
		} else if v, ok := item["mapping_ts_code"].(string); ok {
			mappingTsCode = v
		} else if v, ok := item["mapping_ts_code"].(float64); ok {
			mappingTsCode = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["mapping_ts_code"].(int); ok {
			mappingTsCode = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["mapping_ts_code"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: fut_mapping")
			log.Printf("字段: mapping_ts_code")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["mapping_ts_code"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 mapping_ts_code 类型")
		}
		items[i] = FutMappingItem{
			TsCode: tsCode,
			TradeDate: tradeDate,
			MappingTsCode: mappingTsCode,
		}
	}

	return items, nil
}
