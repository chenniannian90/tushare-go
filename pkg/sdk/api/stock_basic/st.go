...
func St(ctx context.Context, client *sdk.Client, req *StRequest) ([]StItem, error) {
	...
	for i, item := range result.Items {
		...
		// 对 string 类型尝试多种转换
		var impDate string
		if item["imp_date"] == nil {
			// 字段值为 null，使用零值
			impDate = ""
		} else if v, ok := item["imp_date"].(string); ok {
			impDate = v
		...
		if item["st_tpye"] == nil {
			// 字段值为 null，使用零值
			stTpye = ""
		} else if v, ok := item["st_tpye"].(string); ok {
			stTpye = v
		...
		// 对 string 类型尝试多种转换
		var stReason string
		if item["st_reason"] == nil {
			// 字段值为 null，使用零值
			stReason = ""
		} else if v, ok := item["st_reason"].(string); ok {
			stReason = v
		...
		if item["st_explain"] == nil {
			// 字段值为 null，使用零值
			stExplain = ""
		} else if v, ok := item["st_explain"].(string); ok {
			stExplain = v
