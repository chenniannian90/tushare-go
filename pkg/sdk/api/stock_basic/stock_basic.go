...
func StockBasic(ctx context.Context, client *sdk.Client, req *StockBasicRequest) ([]StockBasicItem, error) {
	...
	for i, item := range result.Items {
		...
		if item["cnspell"] == nil {
			// 字段值为 null，使用零值
			cnspell = ""
		} else if v, ok := item["cnspell"].(string); ok {
			cnspell = v
		...
		if item["is_hs"] == nil {
			// 字段值为 null，使用零值
			isHs = ""
		} else if v, ok := item["is_hs"].(string); ok {
			isHs = v
		...
		// 对 string 类型尝试多种转换
		var actName string
		if item["act_name"] == nil {
			// 字段值为 null，使用零值
			actName = ""
		} else if v, ok := item["act_name"].(string); ok {
			actName = v
		...
		if item["act_ent_type"] == nil {
			// 字段值为 null，使用零值
			actEntType = ""
		} else if v, ok := item["act_ent_type"].(string); ok {
			actEntType = v
