// Code generated by tushare-gen. DO NOT EDIT.

package stock_financial

import (
	"context"
	"encoding/json"
	"fmt"
	"log"

	"tushare-go/pkg/sdk"
)

// DividendRequest 表示 分红送股数据 API 的请求
type DividendRequest struct {
	TsCode string `json:"ts_code,omitempty"` // TS代码
	AnnDate string `json:"ann_date,omitempty"` // 公告日
	RecordDate string `json:"record_date,omitempty"` // 股权登记日期
	ExDate string `json:"ex_date,omitempty"` // 除权除息日
	ImpAnnDate string `json:"imp_ann_date,omitempty"` // 实施公告日
}

// DividendItem 表示单个 分红送股数据 数据项
type DividendItem struct {
	TsCode string `json:"ts_code"` // TS代码
	EndDate string `json:"end_date"` // 分红年度
	AnnDate string `json:"ann_date"` // 预案公告日
	DivProc string `json:"div_proc"` // 实施进度
	StkDiv float64 `json:"stk_div"` // 每股送转
	StkBoRate float64 `json:"stk_bo_rate"` // 每股送股比例
	StkCoRate float64 `json:"stk_co_rate"` // 每股转增比例
	CashDiv float64 `json:"cash_div"` // 每股分红（税后）
	CashDivTax float64 `json:"cash_div_tax"` // 每股分红（税前）
	RecordDate string `json:"record_date"` // 股权登记日
	ExDate string `json:"ex_date"` // 除权除息日
	PayDate string `json:"pay_date"` // 派息日
	DivListdate string `json:"div_listdate"` // 红股上市日
	ImpAnnDate string `json:"imp_ann_date"` // 实施公告日
	BaseDate string `json:"base_date"` // 基准日
	BaseShare float64 `json:"base_share"` // 基准股本（万）
}

// Dividend 调用 分红送股数据 API
// 分红送股数据
// 注意：积分要求等使用限制请参考Tushare官方文档
func Dividend(ctx context.Context, client *sdk.Client, req *DividendRequest) ([]DividendItem, error) {
	params := map[string]interface{}{}
	if req.TsCode != "" {
		params["ts_code"] = req.TsCode
	}
	if req.AnnDate != "" {
		params["ann_date"] = req.AnnDate
	}
	if req.RecordDate != "" {
		params["record_date"] = req.RecordDate
	}
	if req.ExDate != "" {
		params["ex_date"] = req.ExDate
	}
	if req.ImpAnnDate != "" {
		params["imp_ann_date"] = req.ImpAnnDate
	}

	fields := []string{"ts_code", "end_date", "ann_date", "div_proc", "stk_div", "stk_bo_rate", "stk_co_rate", "cash_div", "cash_div_tax", "record_date", "ex_date", "pay_date", "div_listdate", "imp_ann_date", "base_date", "base_share"}

	var result struct {
		Fields []string                 `json:"fields"`
		Items  []map[string]interface{} `json:"items"`
	}

	if err := client.CallAPIFlexible(ctx, "dividend", params, fields, &result); err != nil {
		return nil, err
	}
	items := make([]DividendItem, len(result.Items))
	for i, item := range result.Items {
		// 处理 ts_code 的简单类型
		// 对 string 类型尝试多种转换
		var tsCode string
		if item["ts_code"] == nil {
			// 字段值为 null，使用零值
			tsCode = ""
		} else if v, ok := item["ts_code"].(string); ok {
			tsCode = v
		} else if v, ok := item["ts_code"].(float64); ok {
			tsCode = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ts_code"].(int); ok {
			tsCode = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ts_code"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: ts_code")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ts_code"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ts_code 类型")
		}
		// 处理 end_date 的简单类型
		// 对 string 类型尝试多种转换
		var endDate string
		if item["end_date"] == nil {
			// 字段值为 null，使用零值
			endDate = ""
		} else if v, ok := item["end_date"].(string); ok {
			endDate = v
		} else if v, ok := item["end_date"].(float64); ok {
			endDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["end_date"].(int); ok {
			endDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["end_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: end_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["end_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 end_date 类型")
		}
		// 处理 ann_date 的简单类型
		// 对 string 类型尝试多种转换
		var annDate string
		if item["ann_date"] == nil {
			// 字段值为 null，使用零值
			annDate = ""
		} else if v, ok := item["ann_date"].(string); ok {
			annDate = v
		} else if v, ok := item["ann_date"].(float64); ok {
			annDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ann_date"].(int); ok {
			annDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ann_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: ann_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ann_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ann_date 类型")
		}
		// 处理 div_proc 的简单类型
		// 对 string 类型尝试多种转换
		var divProc string
		if item["div_proc"] == nil {
			// 字段值为 null，使用零值
			divProc = ""
		} else if v, ok := item["div_proc"].(string); ok {
			divProc = v
		} else if v, ok := item["div_proc"].(float64); ok {
			divProc = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["div_proc"].(int); ok {
			divProc = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["div_proc"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: div_proc")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["div_proc"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 div_proc 类型")
		}
		// 处理 stk_div 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var stkDiv float64
		if item["stk_div"] == nil {
			// 字段值为 null，使用零值
			stkDiv = 0
		} else if v, ok := item["stk_div"].(float64); ok {
			stkDiv = v
		} else if v, ok := item["stk_div"].(int); ok {
			stkDiv = float64(v)
		} else if v, ok := item["stk_div"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				stkDiv = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					stkDiv = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["stk_div"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: dividend")
					log.Printf("字段: stk_div")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["stk_div"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 stk_div 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["stk_div"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: stk_div")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["stk_div"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 stk_div 类型，期望 float64/int/string")
		}
		// 处理 stk_bo_rate 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var stkBoRate float64
		if item["stk_bo_rate"] == nil {
			// 字段值为 null，使用零值
			stkBoRate = 0
		} else if v, ok := item["stk_bo_rate"].(float64); ok {
			stkBoRate = v
		} else if v, ok := item["stk_bo_rate"].(int); ok {
			stkBoRate = float64(v)
		} else if v, ok := item["stk_bo_rate"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				stkBoRate = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					stkBoRate = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["stk_bo_rate"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: dividend")
					log.Printf("字段: stk_bo_rate")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["stk_bo_rate"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 stk_bo_rate 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["stk_bo_rate"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: stk_bo_rate")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["stk_bo_rate"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 stk_bo_rate 类型，期望 float64/int/string")
		}
		// 处理 stk_co_rate 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var stkCoRate float64
		if item["stk_co_rate"] == nil {
			// 字段值为 null，使用零值
			stkCoRate = 0
		} else if v, ok := item["stk_co_rate"].(float64); ok {
			stkCoRate = v
		} else if v, ok := item["stk_co_rate"].(int); ok {
			stkCoRate = float64(v)
		} else if v, ok := item["stk_co_rate"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				stkCoRate = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					stkCoRate = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["stk_co_rate"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: dividend")
					log.Printf("字段: stk_co_rate")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["stk_co_rate"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 stk_co_rate 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["stk_co_rate"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: stk_co_rate")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["stk_co_rate"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 stk_co_rate 类型，期望 float64/int/string")
		}
		// 处理 cash_div 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var cashDiv float64
		if item["cash_div"] == nil {
			// 字段值为 null，使用零值
			cashDiv = 0
		} else if v, ok := item["cash_div"].(float64); ok {
			cashDiv = v
		} else if v, ok := item["cash_div"].(int); ok {
			cashDiv = float64(v)
		} else if v, ok := item["cash_div"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				cashDiv = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					cashDiv = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["cash_div"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: dividend")
					log.Printf("字段: cash_div")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["cash_div"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 cash_div 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["cash_div"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: cash_div")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["cash_div"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 cash_div 类型，期望 float64/int/string")
		}
		// 处理 cash_div_tax 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var cashDivTax float64
		if item["cash_div_tax"] == nil {
			// 字段值为 null，使用零值
			cashDivTax = 0
		} else if v, ok := item["cash_div_tax"].(float64); ok {
			cashDivTax = v
		} else if v, ok := item["cash_div_tax"].(int); ok {
			cashDivTax = float64(v)
		} else if v, ok := item["cash_div_tax"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				cashDivTax = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					cashDivTax = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["cash_div_tax"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: dividend")
					log.Printf("字段: cash_div_tax")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["cash_div_tax"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 cash_div_tax 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["cash_div_tax"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: cash_div_tax")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["cash_div_tax"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 cash_div_tax 类型，期望 float64/int/string")
		}
		// 处理 record_date 的简单类型
		// 对 string 类型尝试多种转换
		var recordDate string
		if item["record_date"] == nil {
			// 字段值为 null，使用零值
			recordDate = ""
		} else if v, ok := item["record_date"].(string); ok {
			recordDate = v
		} else if v, ok := item["record_date"].(float64); ok {
			recordDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["record_date"].(int); ok {
			recordDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["record_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: record_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["record_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 record_date 类型")
		}
		// 处理 ex_date 的简单类型
		// 对 string 类型尝试多种转换
		var exDate string
		if item["ex_date"] == nil {
			// 字段值为 null，使用零值
			exDate = ""
		} else if v, ok := item["ex_date"].(string); ok {
			exDate = v
		} else if v, ok := item["ex_date"].(float64); ok {
			exDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["ex_date"].(int); ok {
			exDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["ex_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: ex_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["ex_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 ex_date 类型")
		}
		// 处理 pay_date 的简单类型
		// 对 string 类型尝试多种转换
		var payDate string
		if item["pay_date"] == nil {
			// 字段值为 null，使用零值
			payDate = ""
		} else if v, ok := item["pay_date"].(string); ok {
			payDate = v
		} else if v, ok := item["pay_date"].(float64); ok {
			payDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["pay_date"].(int); ok {
			payDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["pay_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: pay_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["pay_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 pay_date 类型")
		}
		// 处理 div_listdate 的简单类型
		// 对 string 类型尝试多种转换
		var divListdate string
		if item["div_listdate"] == nil {
			// 字段值为 null，使用零值
			divListdate = ""
		} else if v, ok := item["div_listdate"].(string); ok {
			divListdate = v
		} else if v, ok := item["div_listdate"].(float64); ok {
			divListdate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["div_listdate"].(int); ok {
			divListdate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["div_listdate"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: div_listdate")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["div_listdate"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 div_listdate 类型")
		}
		// 处理 imp_ann_date 的简单类型
		// 对 string 类型尝试多种转换
		var impAnnDate string
		if item["imp_ann_date"] == nil {
			// 字段值为 null，使用零值
			impAnnDate = ""
		} else if v, ok := item["imp_ann_date"].(string); ok {
			impAnnDate = v
		} else if v, ok := item["imp_ann_date"].(float64); ok {
			impAnnDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["imp_ann_date"].(int); ok {
			impAnnDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["imp_ann_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: imp_ann_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["imp_ann_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 imp_ann_date 类型")
		}
		// 处理 base_date 的简单类型
		// 对 string 类型尝试多种转换
		var baseDate string
		if item["base_date"] == nil {
			// 字段值为 null，使用零值
			baseDate = ""
		} else if v, ok := item["base_date"].(string); ok {
			baseDate = v
		} else if v, ok := item["base_date"].(float64); ok {
			baseDate = fmt.Sprintf("%.0f", v)
		} else if v, ok := item["base_date"].(int); ok {
			baseDate = fmt.Sprintf("%d", v)
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["base_date"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: base_date")
			log.Printf("错误: 类型转换失败，期望类型 string，支持 string/float64/int")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["base_date"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 base_date 类型")
		}
		// 处理 base_share 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var baseShare float64
		if item["base_share"] == nil {
			// 字段值为 null，使用零值
			baseShare = 0
		} else if v, ok := item["base_share"].(float64); ok {
			baseShare = v
		} else if v, ok := item["base_share"].(int); ok {
			baseShare = float64(v)
		} else if v, ok := item["base_share"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				baseShare = 0
			} else {
				// 使用 fmt.Sscanf 解析字符串
				var parsed float64
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					baseShare = parsed
				} else {
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["base_share"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: dividend")
					log.Printf("字段: base_share")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["base_share"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 base_share 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			fieldJSON, _ := json.Marshal(item["base_share"])
			log.Printf("=== 字段解析失败 ===")
			log.Printf("API: dividend")
			log.Printf("字段: base_share")
			log.Printf("错误: 类型转换失败，期望类型 float64，支持 float64/int/string")
			log.Printf("字段原始值: %s", string(fieldJSON))
			log.Printf("字段实际类型: %T", item["base_share"])
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 base_share 类型，期望 float64/int/string")
		}
		items[i] = DividendItem{
			TsCode: tsCode,
			EndDate: endDate,
			AnnDate: annDate,
			DivProc: divProc,
			StkDiv: stkDiv,
			StkBoRate: stkBoRate,
			StkCoRate: stkCoRate,
			CashDiv: cashDiv,
			CashDivTax: cashDivTax,
			RecordDate: recordDate,
			ExDate: exDate,
			PayDate: payDate,
			DivListdate: divListdate,
			ImpAnnDate: impAnnDate,
			BaseDate: baseDate,
			BaseShare: baseShare,
		}
	}

	return items, nil
}
