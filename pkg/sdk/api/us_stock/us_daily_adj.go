...
func UsDailyAdj(ctx context.Context, client *sdk.Client, req *UsDailyAdjRequest) ([]UsDailyAdjItem, error) {
	...
	for i, item := range result.Items {
		...
		if item["free_mv"] == nil {
			// 字段值为 null，使用零值
			freeMv = 0
		} else if v, ok := item["free_mv"].(float64); ok {
			freeMv = v
		} else if v, ok := item["free_mv"].(int); ok {
			freeMv = float64(v)
		} else if v, ok := item["free_mv"].(string); ok {
			// 尝试解析字符串
			if v == "" {
				freeMv = 0
			} else {
				...
				if _, err := fmt.Sscanf(v, "%f", &parsed); err == nil {
					...
					itemJSON, _ := json.Marshal(item)
					fieldJSON, _ := json.Marshal(item["free_mv"])
					log.Printf("=== 字段解析失败 ===")
					log.Printf("API: us_daily_adj")
					log.Printf("字段: free_mv")
					log.Printf("错误: 无法解析字符串为 float64")
					log.Printf("字段原始值: %s", string(fieldJSON))
					log.Printf("字段实际类型: %T", item["free_mv"])
					log.Printf("当前Item: %s", string(itemJSON))
					log.Printf("===================")
					return nil, fmt.Errorf("无效的 free_mv 类型: 无法解析字符串 %q", v)
				}
			}
		} else {
			itemJSON, _ := json.Marshal(item)
			...
			log.Printf("当前Item: %s", string(itemJSON))
			log.Printf("===================")
			return nil, fmt.Errorf("无效的 free_mv 类型，期望 float64/int/string")
		}
		// 处理 total_mv 的简单类型
		// 处理 float64 类型 - 支持多种输入格式
		var totalMv float64
